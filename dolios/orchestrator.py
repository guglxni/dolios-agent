...
class DoliosOrchestrator:
    ...
    def _handle_steer_command(self, user_input: str, agent: Any, console: Any) -> bool:
        ...
        if self._scan_content_for_injection(steer_text, "steer_message") is None:
            audit_logger.record(
                ...
                tool_name="steer_input",
                args={},
                policy_decision="blocked",
                reason="Steer message matched injection pattern",
            )
            console.print(
        ...
        if accepted:
            ...
            audit_logger.record(
                ...
                tool_name="agent_steer",
                args={},
                policy_decision="allowed",
                reason="Mid-turn steer accepted",
            )
        else:
    ...
    def _handle_model_command(self, user_input: str, agent: Any, console: Any) -> bool:
        ...
        audit_logger.record(
            ...
            tool_name="agent_switch_model",
            args={"provider": route.provider, "model": route.model},
            policy_decision="allowed",
            reason=route.reason,
        )
        return True
